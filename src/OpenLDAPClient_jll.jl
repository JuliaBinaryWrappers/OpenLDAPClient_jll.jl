...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import OpenLDAPClient_jll on Julia versions older than 1.3!")
end
...
if isdir(override_dir)
    ...
    function find_artifact_dir()
        return artifact"OpenLDAPClient"
    end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("OpenLDAPClient_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "OpenLDAPClient_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"OpenLDAPClient", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "OpenLDAPClient_jll", "src", "OpenLDAPClient_jll.jl"))
        @info("OpenLDAPClient_ll dev'ed out to /depot/dev/OpenLDAPClient_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("234f2d50-f964-585e-b1c8-17cd4567e563"))
platforms = [Pkg.Artifacts.unpack_platform(e, "OpenLDAPClient", artifacts_toml) for e in artifacts["OpenLDAPClient"]]
...
if best_platform === nothing
    @debug("Unable to load OpenLDAPClient; unsupported platform $(triplet(platform_key_abi()))")
else
